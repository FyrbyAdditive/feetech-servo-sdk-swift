...
func initializeServo(portHandler: PortHandler, packetHandler: PacketHandlerProtocol) -> Bool {
    ...
    if torqueResult != .success {
        print("✗ Failed to enable torque: \(torqueResult.description)")
        return false
    } else if !torqueError.isEmpty {
        print("✗ Protocol error enabling torque: \(torqueError.description)")
        return false
    } else {
...
print("Update the device path in ReadWriteConfiguration.deviceName if needed.")
print()
...
waitForUserInput("Press Enter to start the read/write example...")
runReadWriteExample()
#endif